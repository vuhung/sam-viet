...
namespace DemoPrediction.Web
{
    public class Prediction
    {
        static int _extension = 1;
        static decimal _minGamma = (decimal) 0.2;
        static decimal _maxGamma = (decimal)0.8;

        public static void DemoPrediction(ref List<decimal> listInput)
        {
            decimal[] arrayInput = listInput.ToArray();
            ForecastTable ft = TimeSeries.adaptiveRateSmoothing(arrayInput, _extension, _minGamma, _maxGamma);
            for (int i = ft.Rows.Count - _extension; i < ft.Rows.Count; i++)
            {
                listInput.Add(Convert.ToDecimal(ft.Rows[i]["Forecast"]));
            }
        }
